...
\begin{frame}[plain]
  \titlepage
  \bottomnote{\url{www.cgranade.com/research/talks/iqc/01-26-2015/} \textbullet~\shortdoi{abc} $\to$ \url{doi.org/abc}}
...
\begin{frame}{Building Large Systems: Computational Limits}
  ...
  \begin{itemize}[<+->]
    ...
    \item Calibration of controls
    \item Verification of controls
  \end{itemize}

\end{frame}

\begin{frame}{Bootstrapping to 50 Qubits}
  ...
  \begin{block}{}
    ...
    large devices, bootstrap up to 50-qubit scale.
  \end{block}
...
\begin{frame}{Outline}
  ...
  \begin{itemize}[<+->]
    \item Particle filters: platform for Bayesian inference
...
\begin{frame}
  ...
  By the Cramer-Rao Bound, error then scales as $\OO(1 / N t_{\text{eq}}^2)$.
...
\begin{frame}{Particle Guess Heuristic}
...
\begin{frame}{Scaling With $N$}
    ...
    We expect from uncompressed quantum Hamiltonian learning that the error
    decays exponentially with more data. This remains the case even with compression.
...
\begin{frame}{Bootstrapping Algorithm}
  ...
  We can learn this with compressed IQLE:
  \begin{itemize}
...
\begin{frame}{Further Information}
  ...
  be found at \url{http://www.cgranade.com/research/talks/iqc/01-26-2015/}.
