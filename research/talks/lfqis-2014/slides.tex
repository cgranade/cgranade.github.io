...
\begin{frame}{Building Large Systems: Computational Limits}
  ...
  \begin{itemize}[<+->]
    ...
    \item \only<-4>{Design of control sequences} \only<5->{\color{gray} \st{Design of control sequences}}
    \item Verification of control
  \end{itemize}

  \uncover<5->{
    Here, we focus on characterization and verification.
  }
...
\begin{frame}{Updating Knowledge}
  ...
  Given a likelihood, we can reason about
  \[
    \Pr(\vec{x} | d, \vec{e}),
  \]
...
\begin{frame}{Bayes Factors and Model Selection}
  ...
  for models $A$, $B$ at each step gives (assuming uniform prior over $A$, $B$)
  $$
    \text{Bayes factor} = \frac{\Pr(D | A; \vec{e})}{\Pr(D | B; \vec{e})}
...
\begin{frame}{IQLE Setup}
  ...
  \uncover<2->{\vskip1.5em
    Simulator:

    \[
        \Qcircuit @R 1em @C 1.5em {
                          & \ustick{t, \vec{y}'_i}             &\ustick{t, \vec{y}_-}                 &        &                &  \\
      \lstick{\ket{\psi}} & \gate{e^{-i H(g'[\vec{y}'_i;\, \vec{z}'_0]) t}} \cwx[-1] &  \gate{e^{+i H(g'[\vec{y}_-;\, \vec{z}_0']) t}} \cwx[-1] & \meter & \rstick{d} \cw &
        }
    \]
    where $g'[\vec{y}_i'; \vec{z}_0'] = g[\vec{y}; \vec{z}_i]$.
  }

\end{frame}

\begin{frame}{Control/Distortion Duality}

  Unknown rescaling example: $g[\vec{y}; \vec{z}] = \vec{y} \odot \vec{z}$.

  Straightforward to invert:
  \[    
    \vec{y}_i' = \vec{y} \odot \vec{z}_i \oslash \vec{z}_0' \quad \Longrightarrow \quad
    g'[\vec{y}_i'; \vec{z}_0'] = g[\vec{y}; \vec{z}_i]
  \]
  \vskip1.5em\pause
  Shows duality between \emph{hypothesis} and \emph{simulation}.
  \[
    \begin{matrix}
      \text{Experiment} & & \text{Simulation} \\ \hline
      \begin{matrix}
        \text{hypothesis}\,\vec{z}_i \\
        \text{fixed}\,\vec{y}
      \end{matrix} & \longleftrightarrow & \begin{matrix}
        \text{fixed}\,\vec{z}_0' \\
        \text{control}\,\vec{y}_i'
      \end{matrix}
    \end{matrix}
  \]

  \begin{block}{}
    QHL: special case $\vec{y} = \vec{1}$.

    What must we invert by to affect $\id$?
  \end{block}

\end{frame}

\begin{frame}{Future: Bootstrapping for Control}

  Same approach can be used to model cross-talk:
  \[
    \vec{z} = (\vecop{\matr{G}}, \vec{\epsilon}) \quad g[\vec{y}; \vec{z}] = \matr{G} \vec{y} + \vec{\epsilon}
  \]
  Parameter count can be reduced by restricting $\matr{G}$ (i.e.: tridiagonal).

  \vskip0.25em

  Extend to learning time-dependent distortions?

  \pause\vskip1em

  Can quantum resources be applied to \emph{design} control?
...
\begin{frame}{Further Information}
  ...
  be found at \url{http://www.cgranade.com/research/talks/lfqis-2014/}.
